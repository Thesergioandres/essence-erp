/**
 * ðŸ›¡ï¸ Middleware de Protección Anti-Producción
 *
...
export const productionWriteGuard = (req, res, next) => {
  ...
  if (!isLocalConnection()) {
    console.error(
      "âŒ BLOQUEADO: Intento de escritura hacia producción detectado",
    );
...
export const validateDatabaseSecurity = () => {
  ...
  if (!localUri) {
    throw new Error(
      "âŒ MONGO_URI_DEV_LOCAL no está configurada.\n" +
  ...
  if (prodUri && prodUri === localUri) {
    throw new Error(
      "âŒ PELIGRO: Las URIs de producción y local son iguales.\n" +
  ...
  if (prodUri) {
    ...
    if (prodUri.includes("w=majority") && !prodUri.includes("readPreference")) {
      console.warn(
        "âš ï¸  ADVERTENCIA: La URI de producción puede tener permisos de escritura.\n" +
  ...
  if (process.env.NODE_ENV === "production") {
    console.warn(
      "âš ï¸  Ejecutando en modo PRODUCCIÓN.\n" +
        "   La sincronización Prodâ†’Local está DESHABILITADA.\n",
    );
  }

  console.warn("[Essence Debug]", "âœ… Configuración de seguridad validada.\n");
  console.warn(
    "[Essence Debug]",
